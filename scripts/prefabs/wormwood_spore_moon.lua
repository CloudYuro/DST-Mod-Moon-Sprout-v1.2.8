...
local function NotAllyTarget(inst)
    if inst:HasOneOfTags(ALLY_TAGS)
    or (inst.components.follower 
    and inst.components.follower:GetLeader() 
    and inst.components.follower:GetLeader():HasTag("player")) then
...
local AREAATTACK_EXCLUDETAGS = { "wall", "spore", "INLIMBO", "notarget", "noattack", "flight", "invisible", "playerghost", "shadow", "brightmare", "moon_spore_protection" }
local TOTAL_EXCLUDETAGS = ConcatArrays(AREAATTACK_EXCLUDETAGS, ALLY_TAGS)

local function onpopped(inst)
    inst.SoundEmitter:PlaySound("dontstarve/common/balloon_pop")
    inst.components.combat:DoAreaAttack(inst, TUNING.MOONSPORE_ATTACK_RANGE, nil, NotAllyTarget, nil, TOTAL_EXCLUDETAGS)
...
local function do_proximity_test(inst)
    ...
    local target = FindEntity(inst, TUNING.MOONSPORE_ATTACK_PROXIMITY, nil, PROXIMITY_MUSTHAVE, TOTAL_EXCLUDETAGS, PROXIMITY_ONEOF)
...
local COLOUR_LIST = {
    {227/255, 227/255, 227/255}, -- 原月亮孢子色
    {1, 0.6, 0.6},   -- 浅红
    {0.6, 1, 0.6},   -- 浅绿
    {0.6, 0.6, 1},   -- 浅蓝
}
...
local function fn()
    ...
    -- local colour = COLOUR_LIST[math.random(2,4)] -- 只用红绿蓝三种浅色
...
return Prefab("wormwood_spore_moon", fn, assets)
