...
local SGwormwood_brightmare_gestalt = require "stategraphs/SGwormwood_brightmare_gestalt"
...
local function GetLevelForTarget(target)
	...
	if target ~= nil then
		...
		if target:HasTag("hostile") or IsEnemyTarget(target) then
			return 3, 0
		end
	end
...
local function Retarget(inst)
	...
	local target = FindEntity(inst, TUNING.GESTALTGUARD_AGGRESSIVE_RANGE, attacktargetcheck, nil, nil, {"_combat"})
...
local function fn()
	...
	inst:AddTag("wormwood_gestalt_guard")
	...
	inst.components.combat:SetDefaultDamage(165)
	...
	-- inst.components.damagetypebonus:AddBonus("shadow_aligned", inst, 1.5, "wormwood_gestalt")
	-- inst.components.damagetyperesist:AddResist("lunar_aligned", inst, 0.5, "wormwood_gestalt")
    ...
    inst:SetStateGraph("SGwormwood_brightmare_gestalt")
    inst:SetBrain(brain)
