...
local function areahitcheck(target, inst)
    return not IsAllyTarget(target)
end
...
local function fn()
    ...
    inst.components.combat:SetAreaDamage(TUNING.DECID_MONSTER_ROOT_ATTACK_RADIUS, 1, areahitcheck)
...
return Prefab("wormwood_deciduous_root", fn, assets)
